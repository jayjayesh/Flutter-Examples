...
class TableViewLoadMoreScreen extends StatefulWidget {
  static const routeName = 'TableViewLoadMoreScreen';

  @override
  _TableViewLoadMoreScreenState createState() =>
      _TableViewLoadMoreScreenState();
}

class _TableViewLoadMoreScreenState extends State<TableViewLoadMoreScreen> {
  ...
  Future<Null> _refreshTableviewGetNewArticles() async {
    return Future.delayed(const Duration(seconds: 2), () {
      String strDateTime = formateDateTime(DateTime.now());
      arrTableViewItems.insert(0, 'item : $strDateTime');
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Tableview Load More'),
        actions: <Widget>[
          new IconButton(
